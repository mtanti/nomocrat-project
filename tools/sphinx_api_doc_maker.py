    ...
    for dir_name in dir_names:
        ...
        if len(child_dir_names) + len(child_code_names) > 0:
            ...
            with open(init_path, 'r', encoding='utf-8') as f:
                ...
                if (
                    not isinstance(tree.body[0].value, ast.Constant)
...
def main(
    ...
    Main function.
    '''
    doc_dir = os.path.abspath(os.path.join(nomocrat_project.path, '..', '..', 'docs', 'source'))

    try:
        os.remove(os.path.join(doc_dir, 'nomocrat_project.rst'))
    except FileNotFoundError:
        pass

    try:
        shutil.rmtree(os.path.join(doc_dir, 'nomocrat_project'))
    except FileNotFoundError:
        pass
...
.. toctree::
    ...
    ''' + (
       '' if (
           set(os.listdir(os.path.join(nomocrat_project.path))) - {'__pycache__'}
           == {'__init__.py'}
       )
       else 'nomocrat_project'
   ) + '''
''', file=f)


    generate(
        os.path.abspath(os.path.join(nomocrat_project.path, '..')),
        doc_dir,
        [],
    )
